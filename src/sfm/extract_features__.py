...
"""
Usage:
    feature_path = extract_features_main(
        conf=feature_conf,
        image_path= images_dir / query,
...
def main(
    ...
    logger.info("Extracting local features with configuration:\n"
                f"{pprint.pformat(conf)}")

    image_path = Path(image_path)
    assert image_path.exists(), f"Image does not exist: {image_path}"
    image_name = image_path.name

    # Create temp directory to simulate dataset root
    temp_root = image_path.parent
    dataset = ImageDataset(temp_root, conf["preprocessing"], paths=[image_name])
    ...
    skip_names = set(
        list_h5_names(export_path)
        if export_path.exists() and not overwrite
        else ()
    )
    dataset.names = [n for n in dataset.names if n not in skip_names]
    if len(dataset.names) == 0:
        logger.info("Skipping the extraction (features already exist).")
        return export_path
        ...
        with h5py.File(str(export_path), "a", libver="latest") as fd:
            try:
                ...
                if "No space left on device" in error.args[0]:
                    logger.error("Out of disk space. Did you enable `as_half`?")
                    del grp, fd[name]
                raise error
